...
double* Hidden(double x, double Wes1[3][8], double T[3]);
double output(double x, double Wes1[3][8], double Wes2[3], double T[3 + 1]);
double Adapt(double Wes2[], double error, double output, double hiddens[]);

int main() {
	setlocale(0, "");
	double Wes1[3][8], Wes2[3], T[3 + 1],
...
double Func(double x) {
	double a = 0.3, b = 0.5, c = 0.05, d = 0.5;
...
double* Hidden(double x, double Wes1[3][8], double T[3]) {
	double* result_value = new double[3];
	for (int i = 0; i < 3; i++) {
		result_value[i] = 0;
	}
	double entrances[8];
